...
// ── API ───────────────────────────────────────────────────────────────────────
import {
  fetchRawPerformance, syncCache, rawPerformanceIdbKey,
  type Creative, type FilterOptions, type RawDailyRow, type CreativeDimensionMap,
} from "@/lib/api";
import { idbDelete } from "@/lib/idb";
import { exportDashboardPdfMeta, type PdfTableRow } from "@/lib/exportPdfMeta";
...
import { GroupingSidebarMeta } from "@/components/GroupingSidebarMeta";
import { type Dim, DEFAULT_HIERARCHY, DIM_META } from "@/lib/hierarchy.meta";
import { DirectoryTreeMeta } from "@/components/DirectoryTreeMeta";
import { TopPerformersMeta } from "@/components/TopPerformersMeta";
import { FilterPanelMeta, type Filters } from "@/components/FilterPanelMeta";
import { ExportModalMeta, type ExportPick } from "@/components/ExportModalMeta";
import { SavedViewsMenu } from "@/components/SavedViewsMenu";
import { CreativeDetailModalMeta } from "@/components/CreativeDetailModalMeta";
...
export const Route = createFileRoute("/dashboard-meta")(({
  ...
  head: () => ({
    meta: [
      { title: "CreativeVisibility — Aukera Jewellery Meta Ads Portal" },
      { name: "description", content: "Real-time visual creative directory and performance reporting for Meta (Facebook/Instagram) Ads campaigns." },
    ],
    links: [
...
const DEFAULT_COLS = {
  impressions: true, clicks: true, cost: true, conversions: true,
  ctr: true, cpc: true, cpm: false, cvr: false, cpl: true,
  landing_page_views: false, thruplays: false, hook_rate: false, video_avg_watch_time: false,
  share_pct: false,
};
...
// Labels for the Top Performers "Rank by" pills (rendered uppercased via CSS)
const RANK_METRIC_LABELS: Record<"ctr" | "conversions" | "cpc" | "cpl", string> = {
  ctr: "CTR",
  conversions: "Leads",
  cpc: "CPC",
  cpl: "CPL",
      ...
      if (threshold?.enabled && threshold.value > 0) {
        ...
        if (hidden.length > 0) {
          ...
          if (isExpanded) {
            ...
            const thumbUrls = hidden.slice(0, 4)
              .map(r => r.creative.creative_url ?? "")
              .filter(Boolean);
...
function Portal() {
  ...
  const { channel_id: channelId } = Route.useSearch();
  ...
  const [rankMetric, setRankMetric]     = useState<"ctr" | "conversions" | "cpc" | "cpl">("ctr");
  ...
  // Note: savedViews.ts's SharedView.hierarchy is typed against the Google
  // Dim[] (shared localStorage/URL-hash format across both dashboards, since
  // it's just persisted strings). Cast to this route's narrower Meta Dim[].
  useEffect(() => {
    const shared = readSharedViewFromHash();
    if (shared) {
      setFilters(shared.filters);
      setHierarchy(shared.hierarchy as unknown as Dim[]);
  ...
  const loadRawData = useCallback(async (quiet = false) => {
    ...
    if (!channelId) {
      setError("No channel selected. Go back to Home and pick an account/channel.");
      setLoading(false);
    ...
    try {
      const data = await fetchRawPerformance(channelId);
  ...
  const handleSync = async () => {
    ...
    setSyncing(true);
    try {
      await syncCache(channelId);
      // Clear this channel's IDB entry only — other channels' cached data is untouched.
      await idbDelete(rawPerformanceIdbKey(channelId));
      await loadRawData();
  ...
  const filteredCreatives = useMemo(() => {
    return creatives.filter(c => {
      ...
      if (filters.search) {
        ...
        const hay = [c.campaign_name, c.ad_group, c.city, c.ad_name, c.headline, c.description]
          .join(" ").toLowerCase();
  ...
  const aggregated = useMemo(() => {
    ...
    for (const c of creatives) {
      result.set(c.creative_id, {
        ...
        // Extra Meta metrics (already aggregated on the creative by aggregator.ts)
        landing_page_views:   c.landing_page_views   ?? 0,
        thruplays:            c.thruplays            ?? 0,
        hook_rate:            c.hook_rate            ?? 0,
        video_avg_watch_time: c.video_avg_watch_time ?? 0,
      });
    }
    return result;
  }, [creatives]);
  ...
  const handleExportCSV = () => {
    const headers = [
      "Creative ID","Ad Name","Campaign","Funnel","Type","City","Status",
      "Impressions","Clicks","Cost","Leads","CTR (%)","CPC (₹)","CPM (₹)","CVR (%)","CPL (₹)",
      "Landing Page Views","ThruPlays","Hook Rate (%)","Avg Watch Time (s)",
    ];
    const rows = visibleRows.map(r => [
      r.creative.creative_id, r.creative.ad_name ?? r.creative.headline ?? "", r.creative.campaign_name,
      r.creative.funnel, r.creative.creative_type, r.creative.city, r.creative.status,
      ...
      r.metrics.cpm, r.metrics.cvr, r.metrics.cpl,
      r.metrics.landing_page_views ?? 0, r.metrics.thruplays ?? 0,
      (r.metrics.hook_rate ?? 0).toFixed(2), (r.metrics.video_avg_watch_time ?? 0).toFixed(1),
    ].map(v => `"${String(v).replace(/"/g, '""')}"`).join(","));
    const csv  = [headers.join(","), ...rows].join("\n");
    const blob = new Blob([csv], { type: "text/csv" });
    const url  = URL.createObjectURL(blob);
    const a    = document.createElement("a");
    a.href = url; a.download = `creativevisibility_meta_${filters.startDate}_${filters.endDate}.csv`;
    ...
    <div id="print-root" className="min-h-screen flex flex-col relative" data-print-theme="light">
      ...
      <header className="app-header border-b border-white/10 sticky top-0 z-30 no-print bg-background/95 backdrop-blur-xl shadow-[0_4px_20px_-8px_rgba(0,0,0,0.6)]">
        <div className="flex items-center gap-4 px-6 py-3">
          ...
          <Link to="/" className="flex items-center gap-2.5 hover:opacity-80 transition-opacity" title="Back to accounts">
            ...
            <div>
              ...
              <div className="text-[10px] text-muted-foreground uppercase tracking-widest">Aukera Jewellery · Meta Ads Portal</div>
            </div>
          </Link>
        ...
        <main className="flex-1 min-w-0 p-6 space-y-5">
          ...
          <div className="sticky top-[60px] z-20 -mx-6 px-6 pt-2 pb-3 backdrop-blur-2xl bg-background/95 border-b border-white/10 shadow-[0_8px_24px_-12px_rgba(0,0,0,0.6)] no-print">
            <FilterPanelMeta
              ...
              rightSlot={
                <SavedViewsMenu
                  current={{ filters, hierarchy, columns, activeKey, selectedIds: Array.from(selected) } as unknown as Parameters<typeof SavedViewsMenu>[0]["current"]}
                  onLoad={(v) => {
                    setFiltersSafe(v.filters);
                    setHierarchy(v.hierarchy as unknown as Dim[]);
              ...
              <div className="no-print flex items-center gap-2 flex-wrap">
                ...
                {activeTab === "top" && (
                  <>
                    ...
                    {(["ctr", "conversions", "cpc", "cpl"] as const).map(m => (
                      <button
                        key={m}
                ...
                {activeTab === "top" && (
                  <div className="ml-auto">
                    ...
                    >
                      ...
                      {topPdfLoading ? <span className="text-[oklch(0.78_0.15_85)]">Generating PDF…</span> : "Download PDF"}
                    </button>
                  </div>
                )}
...
function LoadingProgress({ secs }: { secs: number }) {
  ...
  const msg = secs === 0
    ? "Connecting to Meta Ads data…"
    : secs < 5
    ? "Fetching creative performance data…"
    : secs < 15
    ? "Reading from Meta Ads… hang tight"
    : secs < 35
