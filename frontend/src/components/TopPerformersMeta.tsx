...
import { computeMetrics, type ComputedMetrics, fmtINR, fmtINR0, fmtNum, fmtPct, upgradeMetaImageUrl } from "@/lib/metrics";
import { exportTopPerformersPdfMeta } from "@/lib/exportTopPerformersPdfMeta";
...
interface Props {
  rows: Row[];
  metric: "ctr" | "conversions" | "cpc" | "cpl";
  rowHeight: number;
  dateRange: string;
...
const metricLabel: Record<Props["metric"], string> = {
  ctr: "CTR", conversions: "Leads", cpc: "CPC", cpl: "CPL"
};
const metricFmt: Record<Props["metric"], (v: number) => string> = {
  ctr: fmtPct, conversions: (v) => v.toFixed(1), cpc: fmtINR, cpl: fmtINR,
};
const ascending = (m: Props["metric"]) => m === "cpc" || m === "cpl";
...
export function ThresholdPillMeta({
  value,
  thresholdType,
  onChange,
  onTypeChange,
...
function HoverPreview({
  ...
  const isVideo = row.creative.creative_type === "Video" && !!row.creative.creative_url;

  const previewW = 600;
  const previewH = 337.5;
    ...
    >
      <div className="glass rounded-2xl overflow-hidden border border-gold/20
        ...
        <div className="relative w-full bg-black overflow-hidden" style={{ height: previewH }}>
          {(isImg || isVideo) && (
            <img
              src={upgradeMetaImageUrl(row.creative.creative_url)}
        ...
        <div className="px-3 py-2.5 space-y-0.5">
          ...
          <div className="text-[11px] text-muted-foreground truncate">
            {row.creative.city} · {row.creative.funnel}
          </div>
        </div>
      </div>
    </div>,
...
// ─── TopPerformersMeta (root) ──────────────────────────────────────────────
export function TopPerformersMeta({ rows, metric, rowHeight, dateRange, onCreativeClick, exportRef }: Props) {
  ...
  const handleExportPdf = async () => {
    await exportTopPerformersPdfMeta({
    ...
    <div ref={columnRef} className="glass rounded-2xl p-5">
      <h3 className="font-display font-semibold text-lg mb-4 flex items-center gap-2 min-w-0">
        ...
        <button
          ...
          title="Aggregate creatives with the same URL across campaigns, cities & ad sets"
          className={cn(
          ...
          return (
            ...
            >
              ...
              <div
                ...
                style={{
                  height: thumbH,
                  width: r.creative.creative_url ? "auto" : thumbW,
                  maxWidth: thumbH * 2,
                }}
              ...
              >
                {r.creative.creative_url && (
                  <>
                    <img
                      src={upgradeMetaImageUrl(r.creative.creative_url)}
                      alt=""
                      className="h-full w-auto object-contain transition-transform"
                      style={{ maxHeight: thumbH, maxWidth: thumbH * 2, imageRendering: "auto" }}
                    />
                    {r.creative.creative_type === "Video" && (
              ...
              <div className="flex-1 min-w-0 flex flex-col justify-center gap-1">
                <div className="font-display font-bold text-[14px] leading-tight truncate group-hover:text-gold transition-colors">
                  {r.creative.ad_name || r.creative.headline || r.creative.creative_id}
                </div>
                {/* Row 1: Location */}
