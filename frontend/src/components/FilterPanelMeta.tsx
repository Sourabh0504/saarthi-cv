...
import { Search, Columns3, FileDown, FileText, Loader2, ChevronDown, Check, RotateCcw, Filter } from "lucide-react";
...
const colOptions = [
  ...
  { key: "video_avg_watch_time", label: "Avg Watch Time" },
  { key: "share_pct",   label: "% Share" },
];

const iconBtn =
  "h-8 px-2.5 flex items-center gap-1.5 rounded-md text-[11px] font-medium " +
...
export function FilterPanelMeta({
  ...
  // Dimension-only count — drives the Filters badge
  ...
  // Full count (dimensions + search) — drives Reset visibility
  const activeCount = useMemo(
    () => dimCount + (filters.search.trim() ? 1 : 0),
    [dimCount, filters.search],
  );
  ...
  return (
    <div className="filter-panel relative group/bar no-print">
      ...
      <div className="relative flex items-center gap-1 p-1.5 rounded-xl bg-[oklch(0.12_0.005_260)] border border-white/[0.06] shadow-[0_10px_40px_-15px_oklch(0_0_0/0.7)] backdrop-blur-xl overflow-x-auto no-scrollbar">
        ...
        {/* ── Consolidated dimension filters ── */}
        <FiltersPopover
          filters={filters}
          update={update}
          cities={cities}
        ...
        <Popover>
          <PopoverTrigger asChild>
            <button type="button" className={iconBtn} title="Toggle columns">
        ...
        {/* ── CSV export ── */}
        <button type="button" onClick={onExportCSV} className={iconBtn} title="Export CSV">
        ...
        {activeCount > 0 && (
          <button
            ...
            onClick={resetAll}
            className={cn(
              "group/reset relative h-8 w-8 flex items-center justify-center rounded-md shrink-0",
              "border border-transparent text-foreground/70 hover:text-gold",
              "hover:bg-white/[0.05] hover:border-[var(--gold)]/20 transition-all cursor-pointer active:scale-95",
            )}
            title={`Reset ${activeCount} filter${activeCount > 1 ? "s" : ""}`}
          >
            <RotateCcw className="w-3.5 h-3.5 transition-transform duration-500 group-hover/reset:-rotate-180" />
        ...
        <button
          ...
          disabled={pdfLoading}
          className={cn(
            "h-8 px-3.5 flex items-center gap-1.5 rounded-md text-[11px] font-bold uppercase tracking-widest shrink-0",
...
// ── Consolidated filters popover (single trigger for all 5 dimensions) ────────
function FiltersPopover({ filters, update, cities, campaignTypes, campaignNames, dimCount }: {
  filters:       Filters;
    ...
    <Popover>
      <PopoverTrigger asChild>
        <button
          ...
          className={cn(
            iconBtn,
            dimCount > 0 && "border-gold/30 bg-gold/[0.06] text-gold hover:bg-gold/[0.10]",
          )}
        ...
        >
          ...
          <span>Filters</span>
          {dimCount > 0 && (
            <span className="w-4 h-4 rounded-full bg-gold-gradient text-[9px] font-bold text-primary-foreground flex items-center justify-center shrink-0">
              {dimCount}
            </span>
          )}
        </button>
      </PopoverTrigger>
      <PopoverContent align="start" sideOffset={6} className="w-72 p-3 space-y-1">
        <FilterRow
          label="Status"
...
// ── Single filter row: label + inline multi-select dropdown ───────────────────
function FilterRow({ label, value, onChange, options, allLabel, searchable }: {
  label:       string;
  value:       string[];
      ...
      <Popover open={open} onOpenChange={handleOpenChange}>
        <PopoverTrigger asChild>
          <button
            type="button"
            className={cn(
              "flex-1 h-8 px-2.5 flex items-center justify-between gap-1.5 rounded-md text-xs border transition-all cursor-pointer",
              hasFilter
                ? "border-gold/30 bg-gold/[0.06] text-gold hover:bg-gold/[0.10]"
                : "border-white/10 bg-white/[0.03] text-foreground/80 hover:bg-white/[0.06] hover:border-white/20",
            )}
          >
            <span className="truncate text-left flex-1">{valueLabel}</span>
            <span className="shrink-0 flex items-center gap-1">
              {hasFilter && value.length > 1 && (
                <span className="w-4 h-4 rounded-full bg-gold-gradient text-[8px] font-bold text-primary-foreground flex items-center justify-center">
                  {value.length}
                </span>
              )}
              {hasFilter && value.length === 1 && (
              ...
              {!hasFilter && (
                <ChevronDown className={cn("w-3 h-3 text-muted-foreground/50 transition-transform duration-200", open && "rotate-180")} />
              )}
            </span>
          </button>
        </PopoverTrigger>

        <PopoverContent align="start" sideOffset={4} className="p-1 min-w-[180px] w-auto max-h-60 flex flex-col">
          {searchable && (
