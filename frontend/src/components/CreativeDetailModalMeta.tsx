...
import { computeMetrics, fmtINR, fmtINR0, fmtNum, fmtPct, upgradeMetaImageUrl } from "@/lib/metrics";
import {
...
import { DIM_META, type Dim } from "@/lib/hierarchy.meta";
import { cn, copyText } from "@/lib/utils";
import { Button } from "@/components/ui/button";
import { Popover, PopoverContent, PopoverTrigger } from "@/components/ui/popover";
import { Check } from "lucide-react";
import { exportCreativePdfMeta as exportCreativePdf } from "@/lib/exportPdfMeta";
...
export function CreativeDetailModalMeta({
  ...
  // Meta: creative_url is the preview image for both Image and Video — no
  // YouTube embed is possible, so video playback just links out to the CDN URL.
  ...
  return (
    <Dialog open={!!creative} onOpenChange={(o) => !o && onClose()}>
      <DialogContent className="w-[960px] max-w-[96vw] max-h-[92vh] overflow-y-auto overflow-x-hidden [&>*]:min-w-0">
        <DialogHeader>
          ...
          <div className="flex items-center gap-1 -mt-1 mb-1 flex-wrap">
            ...
            >
              {isExporting
                ? <><Loader2 className="w-3.5 h-3.5 animate-spin text-[oklch(0.78_0.15_85)]" /><span className="text-[oklch(0.78_0.15_85)]">Exporting…</span></>
          ...
          <div className="space-y-1.5 pr-8">
            ...
            <div className="flex items-center gap-4 flex-wrap">
              ...
              <span className="flex items-center gap-1.5 min-w-0 max-w-full">
                <span className="text-[11px] font-bold uppercase tracking-widest text-muted-foreground/70 shrink-0">Ad Set</span>
                <span className="font-mono text-sm text-foreground/90 break-all">{creative.ad_group || "--"}</span>
              </span>
            </div>
          </div>

          {/* Title — clickable link to the creative asset */}
          <DialogTitle className="font-display text-xl tracking-tight pr-8 pt-1 break-words">
            <a
              href={creative.creative_url}
              target="_blank"
              rel="noopener noreferrer"
              className="inline-flex items-center gap-2 hover:text-gold transition-colors group max-w-full"
            >
              <span className="underline-offset-4 group-hover:underline break-words min-w-0">{adName(creative)}</span>
        ...
        <div className="flex flex-wrap gap-4 pt-2 items-start">
          ...
          <div className="rounded-xl overflow-hidden border border-border bg-muted/40">
            ...
            {creative.creative_type === "Video" && creative.creative_url && (
              <div className="relative w-[280px] aspect-video bg-black flex items-center justify-center">
                ...
                />
                <div className="absolute inset-0 bg-black/35 hover:bg-black/45 transition-colors" />
                {/* Play button — opens URL externally since Meta CDN links aren't embeddable */}
                <a
                  href={creative.creative_url}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="absolute w-14 h-14 rounded-full bg-black/60 border border-white/20 flex items-center justify-center
                             backdrop-blur-sm hover:scale-110 hover:bg-gold/90 hover:border-gold transition-all duration-200 pointer-events-auto"
                >
                  <Play className="w-6 h-6 text-white hover:text-black fill-current ml-0.5" />
                </a>
              </div>
            )}
            {creative.creative_type === "Image" && !creative.creative_url && (
          ...
          <div className="flex-1 min-w-0 space-y-2">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
              ...
              {modalMetrics.conversions && <Stat label="Leads" value={totals.conversions.toFixed(1)} />}
              {modalMetrics.cvr         && <Stat label="CVR"   value={fmtPct(totals.cvr)} />}
              {modalMetrics.cpl         && <Stat label="CPL"   value={fmtINR(totals.cpl)} />}
            </div>
