...
import { computeMetrics, fmtINR0, fmtNum, fmtPct, fmtINR, upgradeMetaImageUrl } from "@/lib/metrics";
...
const COL_LABELS: Record<string, string> = {
  ...
  conversions: "LEADS", ctr:    "CTR",       cpc:  "CPC",
  cpm:         "CPM",   cvr:    "CVR",        cpl:  "CPL", share_pct: "% SHARE",
  landing_page_views: "LPV", thruplays: "THRUPLAY", hook_rate: "HOOK %", video_avg_watch_time: "WATCH S",
};
...
const METRIC_FMT: Record<string, (m: ComputedMetrics) => string> = {
  ...
  cvr:         m => fmtPct(m.cvr),
  cpl:         m => fmtINR(m.cpl),
  landing_page_views:   m => fmtNum(m.landing_page_views ?? 0),
  thruplays:            m => fmtNum(m.thruplays ?? 0),
  hook_rate:            m => fmtPct(m.hook_rate ?? 0),
  video_avg_watch_time: m => (m.video_avg_watch_time ?? 0).toFixed(1) + "s",
  share_pct:   () => "—",
};
  ...
  return (
    <div style={{ background: C.bg, fontFamily: "'Poppins', sans-serif", color: C.text, fontSize: 10, lineHeight: "1.4" }}>
      ...
      {previewGroups.map((group, gi) => {
        ...
        const rawUrl = (creative?.creative_type === "Image" || creative?.creative_type === "Video")
          ? (creative.creative_url ?? null)
          : null;
        const imgSrc = upgradeMetaImageUrl(rawUrl);
              ...
              <div style={{
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ fontSize: 7, color: C.text, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                    {creative.ad_name || creative.headline || creative.creative_url.replace(/^https?:\/\//, "").slice(0, 45)}
                  </div>
...
export function ExportModalMeta({ open, onClose, onPick, context, visibleRows, totals, hierarchy, activeLevel }: Props) {
