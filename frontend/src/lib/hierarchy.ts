...
export const DIM_META: Record<Dim, {
  ...
  creative: { label: "Creative",  short: "CRV",  get: c => c.creative_url || c.headline || c.creative_id, icon: Sparkles },
};

export const ALL_DIMS: Dim[] = ["city", "funnel", "type", "campaign", "adgroup", "creative"];

// Creative is now an explicit dim — included at the end by default so users can drag it anywhere.
...
export const HIERARCHY_PRESETS: { id: string; label: string; dims: Dim[] }[] = [
  ...
  { id: "creative-first", label: "Creative › Location › Funnel", dims: ["creative", "city", "funnel", "type", "campaign", "adgroup"] },
];
