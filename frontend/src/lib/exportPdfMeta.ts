/**
 * frontend/src/lib/exportPdfMeta.ts
 * ===================================
 * Meta Ads variant of exportPdf.ts — same jsPDF + svg2pdf.js vector-PDF
 * architecture, adapted for Meta's data model:
 *   - CVR / CPL instead of CR / CPA (same underlying formulas)
 *   - Thumbnails always come from `creative_url` (Meta CDN), never YouTube
 *
 * Architecture:
...
import { fmtINR, fmtINR0, fmtNum, fmtPct, upgradeMetaImageUrl } from "@/lib/metrics";
...
export async function exportCreativePdfMeta(data: CreativePdfData): Promise<void> {
  ...
  // Meta: creative_url is the preview image for both Image and Video (no YouTube).
  let imgSrc: string | null = null;
  if (creative.creative_type === "Image" || creative.creative_type === "Video") {
    imgSrc = creative.creative_url ? upgradeMetaImageUrl(creative.creative_url) : null;
  }
  ...
  const ALL_METRIC_CELLS: Array<MetricCell & { key: string }> = [
    ...
    { key: "conversions", label: "Leads",       value: totals.conversions.toFixed(1) },
    { key: "cvr",         label: "Conv. Rate",  value: fmtPct(totals.cvr) },
  ];
  ...
  const adTitle = creative.ad_name || creative.headline
    || (creative.creative_type === "Text" ? "Text Ad"
      : [creative.creative_type, creative.city, creative.ad_group].filter(Boolean).join(" · "));
...
export async function exportDashboardPdfMeta(data: DashboardPdfData): Promise<void> {
  const { jsPDF } = await import("jspdf");
  ...
  await Promise.all([
    ...creativeRows.map(async (r) => {
      ...
      const src = (c.creative_type === "Image" || c.creative_type === "Video") && c.creative_url
        ? upgradeMetaImageUrl(c.creative_url)
        : null;
      if (!src) return;
  ...
  const drawMetrics = (m: ComputedMetrics, ry: number, rh: number, bold: boolean, total: boolean) => {
    for (let i = 0; i < enabledColumns.length; i++) {
      ...
      switch (key) {
        ...
        case "cvr":         val = fmtPct(m.cvr);               break;
        case "cpl":         val = safe(fmtINR(m.cpl));         break;
        case "landing_page_views":   val = fmtNum(m.landing_page_views ?? 0);              break;
        case "thruplays":            val = fmtNum(m.thruplays ?? 0);                       break;
        case "hook_rate":            val = fmtPct(m.hook_rate ?? 0);                       break;
        case "video_avg_watch_time": val = (m.video_avg_watch_time ?? 0).toFixed(1) + "s"; break;
        default:            val = "—";
      }
  ...
  const COL_ABBR: Record<string,string> = {
    impressions:"IMPR.",clicks:"CLICKS",cost:"SPEND",conversions:"LEADS",
    ctr:"CTR",cpc:"CPC",cpm:"CPM",cvr:"CVR",cpl:"CPL",share_pct:"%SHR",
    landing_page_views:"LPV",thruplays:"THRU",hook_rate:"HOOK%",video_avg_watch_time:"WATCH",
  };
...
export async function exportPdfMeta({
  theme,
  dateRange = "",
